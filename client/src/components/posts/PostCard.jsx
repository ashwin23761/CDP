...
export default function PostCard({ post, onPostDeleted, onTagClick }) {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await createComment({ post_id, content: newComment.trim() });
      if (response.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        setDeleted(true);
        if (onPostDeleted) onPostDeleted(post_id);
      }
  ...
  return (
    <article
      data-testid={`post-card-${post_id}`}
      className="post-card group relative rounded-xl border border-[#1C1C2E] bg-[#101018] overflow-hidden hover:border-[#2E2E42] hover:bg-[#13131F] transition-all duration-300"
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex flex-col leading-tight flex-1">
            <Link to={`/profile/${user_id}`} className="text-xs text-[#52526E] font-body hover:text-[#C8FF00] transition-colors">
          ...
          {isAuthor && (
            <div className="relative">
              <button data-testid={`post-menu-btn-${post_id}`} onClick={() => setShowMenu(!showMenu)} className="w-7 h-7 rounded-full border border-[#1C1C2E] hover:border-[#C8FF00] flex items-center justify-center text-[#52526E] hover:text-[#C8FF00] transition-all text-xs">
                ···
              </button>
              {showMenu && (
                <div className="absolute right-0 top-9 z-20 w-32 rounded-lg border border-[#1C1C2E] bg-[#101018] shadow-lg overflow-hidden" style={{ animation: 'fadeUp 0.15s ease forwards' }}>
                  <button data-testid={`edit-post-btn-${post_id}`} onClick={() => { setEditing(true); setShowMenu(false); }} className="w-full text-left px-4 py-2.5 text-xs text-[#F0F0FF] hover:bg-[#1C1C2E] font-body transition-colors">
                    Edit
                  </button>
                  <button data-testid={`delete-post-btn-${post_id}`} onClick={() => { handleDelete(); setShowMenu(false); }} className="w-full text-left px-4 py-2.5 text-xs text-red-400 hover:bg-red-900/20 font-body transition-colors">
                    Delete
                  </button>
                </div>
              )}
          ...
          <>
            ...
            {tagList.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {tagList.map((tag, i) => (
                  <button key={i} onClick={() => onTagClick && onTagClick(tag)} className="px-2 py-0.5 rounded text-[10px] font-body bg-[#1C1C2E] text-[#C8FF00] hover:bg-[#C8FF00] hover:text-[#090910] transition-colors">
                    #{tag}
                  </button>
                ))}
              </div>
          ...
          <div className="mt-4 pt-4 border-t border-[#1C1C2E]">
            <form onSubmit={handleCommentSubmit} className="mb-4">
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={!newComment.trim() || submittingComment} data-testid={`comment-submit-${post_id}`} className="px-4 py-2 text-sm rounded-lg bg-[#C8FF00] text-[#090910] font-bold hover:bg-[#d8ff33] disabled:opacity-50 disabled:cursor-not-allowed transition-all font-body">
              ...
              <div className="space-y-3">
                {comments.map((comment) => (
                  <div key={comment.comment_id} className="flex gap-2">
                    <Link to={`/profile/${comment.user_id}`} className="w-6 h-6 rounded-full flex items-center justify-center text-[#090910] text-[10px] font-bold shrink-0" style={{ backgroundColor: avatarColor(comment.user_id) }}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-[#F0F0FF] font-body">{comment.content}</p>
                    </div>
                  </div>
                ))}
              </div>
