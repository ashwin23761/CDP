...
export default function PostList({ groupId, newPost }) {
  ...
  if (loading) return <div className="flex flex-col gap-4">{[1, 2, 3].map((n) => <SkeletonCard key={n} />)}</div>;
  if (error) return <div className="rounded-xl border border-red-900 bg-red-950/30 p-6 text-center"><p className="text-red-400 font-body text-sm">{error}</p></div>;
